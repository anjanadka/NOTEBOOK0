...
// User model
let Textbook = require('../models/textbook');
...
router.post('/file-upload/:tid&&:cid', upload.single('files'), (req, res, next) => {
  ...
  const textbook = new Textbook({
    ...
    desc : req.body.desc,
    date : req.body.date,
    files: url + '/public/textbook/' + req.file.filename,
...
router.route('/find_tr/:tid&&:cid').get((req, res) => {
  Textbook.find({teacher_id:req.params.tid,class:req.params.cid},(error, data) => {
   if (error) {
     return next(error)
   } else {
     res.json(data)
