...
<div>
  ...
  <v-card class="auto" max-width="100%" tile>
    <v-list rounded>
      <v-subheader><v-toolbar color="green" dark><h2>ASSIGNMENT</h2></v-toolbar></v-subheader>
      <br><br>
      <v-list-item-group>
          ...
          <v-layout color="black" dark align-center justify-center flat>
            <v-card class="auto" color="grey lighten-3" width="100%">
              <v-list-item-content>
               ...
               <div v-if="s==1">
                  <v-list-item :key="assign._id" >
                <v-list-item-title >status : SUBMITTED</v-list-item-title><br /><br />
                <v-list-item-title v-if="assign.mark != -1">Mark : {{assign.mark}}</v-list-item-title><br />
                <v-list-item-title v-if="assign.mark == -1">Mark : Result not Published</v-list-item-title><br />
                  </v-list-item>
                  </div>
                  <div v-if="s==0">
                  <v-list-item>
...
export default {
  ...
  created() {
    let apiURL = `http://localhost:2000/api/Assignment/one_assignment_find/${this.$route.params.aid}`;
    axios
      .get(apiURL)
      .then((res) => {
        this.item = res.data;
      ...
      .catch((error) => {
       ...
       let apiURL1 =`http://localhost:2000/api/Assignment_student/find_sassign/${this.$route.params.sid}&&${this.$route.params.aid}`;
      axios
      .get(apiURL1)
      .then((res) => {
  ...
  getDate(test) {
    ...
    var res1=t.split("-"); 
     ...
     handleSubmit() {
          ...
          axios.post(`http://localhost:2000/api/Assignment_student/file-upload/${this.$route.params.sid}&&${this.$route.params.cid}&&${this.$route.params.aid}`, formData, {
          }).then((res) => {
            this.$router.push(
            `/s_assignment/${this.$route.params.sid}&&${this.$route.params.cid}&&${this.$route.params.tid}`
          );
             this.files = null;
