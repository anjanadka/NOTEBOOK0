 ...
 <div class="row" height=0>
  ...
  <div class="col-sm-8">
        <v-toolbar color="blue darken-4" dark flat>
                <v-toolbar-title class="text-uppercase">previous year question paper</v-toolbar-title>
        </v-toolbar>
          <v-container fluid fill-height>
               <v-layout align-center justify-center>
                    <v-flex xs12 sm12 md6>
                         ...
                         <v-card-actions>
                              <v-spacer></v-spacer>
                             <v-btn dark color="blue darken-4" @click="handleSubmit">SAVE</v-btn>
                         </v-card-actions>
                         <br><br>
                    </v-flex>
               </v-layout>
  ...
  export default {
    ...
    methods: {
        handleSubmit() {
          ...
          axios.post(`http://localhost:2000/api/QuestionPaper/file-upload/${this.$route.params.tid}&&${this.$route.params.cid}`, formData, {
          }).then((res) => {               
