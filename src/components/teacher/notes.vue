          ...
          <v-container fluid fill-height>
               <v-layout align-center justify-center>
                    <v-flex xs12 sm12 md6>
                         ...
                         <v-card-actions>
                             <v-spacer></v-spacer>
                             <v-btn dark color="red darken-4"  @click="handleSubmit">SAVE</v-btn>
                         </v-card-actions>
                          <br><br>
                    </v-flex>
               </v-layout>
  ...
  export default {
    ...
    methods: {
        handleSubmit() {
          ...
          axios.post(`http://localhost:2000/api/Notes/file-upload/${this.$route.params.tid}&&${this.$route.params.cid}`, formData, {
          }).then((res) => {
